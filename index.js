...
//console.log(removeArtist(artists, 0)); <--console.log commented out to pass testing
...
function addArtist(array, name, years, genre, nationality, bio) {
  let upAr = array.slice();
  upAr.push({name, years, genre, nationality, bio});
  return upAr;
}

console.log(addArtist(artists, "John Doe", "1988-2022", "Full Stack Development", "African American", "I have a backgrounf in customer service at Big Retail Chain. I am attending Bloomtech to become a Frontend Developer."));
...
function lotsOfArt(ar) {
  ...
  for (let i = 0; i < ar.length; i++){
    if (ar[i].paintings >= 100){
      onlyNames.push(ar[i].name);
    }
...
function artistInfo(ara, nami){
let holder = [];
  for (let i = 0; i < ara.lenth; i++){
    if (ara[i].name === nami){
      holder.push(ara[i].bio);
    }
  }
  return holder;
//  return bioA;
...
function artistByCountry(list, string1){
  let coit = [];
  for (let i = 0; i < list.length; i++){
    if (list[i].nationality === string1){
      coit.push(list[i].name);
    }
  }
  return coit;
}
